# Third-Party Notices

Clip Image Viewer includes third-party components under their own licenses.

- Electron: MIT
- Sharp/libvips: Apache-2.0 and LGPL components
- ImageMagick and magick-wasm: ImageMagick License and Apache-2.0.
  ImageMagick is Copyright © 1999 ImageMagick Studio LLC.
- LibRaw: LGPL-2.1 or CDDL-1.0
- JPEG XR decoder/jxrlib: Apache-2.0 and BSD licenses
- BPG decoder/libbpg: ISC, BSD and LGPL-2.1 components
- sql.js/SQLite: MIT and public domain
- ag-psd, @webtoon/psd, adm-zip: package-specific open-source licenses

Complete license texts and package notices are distributed in the `licenses/`
directory, including `licenses/npm/` for runtime npm packages.
