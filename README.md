...
- PSD 레이어별 썸네일과 목록, 표시 ON/OFF, 혼합 모드·불투명도·클리핑·마스크·효과 확인
- `Ctrl+클릭`으로 PSD 이미지의 해당 픽셀을 포함하는 레이어 선택
- CLIP/CSP 래스터·벡터 경로·용지·텍스트·채우기 레이어 미리보기와 빠른 표시 ON/OFF
- CLIP/CSP 3D·조정 필터·CSP 전용 레이어 문서 문맥 미리보기
...
- CLIP/CSP의 저장된 래스터 mipmap, 벡터 스트로크, 용지, 기본 텍스트,
  단색·그레이디언트 채우기는 직접 재구성해 ON/OFF합니다.
- 3D와 조정 필터 등 CELSYS의 비공개 렌더러가 필요한 레이어는 문서 문맥
  미리보기로 표시되며 독립 합성 ON/OFF가 비활성화됩니다.
- CLIP/CSP 벡터 브러시 질감, 텍스트 변형·테두리와 고유 효과는 CSP 결과와 다를 수 있으며
  이미지 위치 기반 레이어 선택은 지원하지 않습니다.
