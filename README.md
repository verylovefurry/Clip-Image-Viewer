...
- CLIP/CSP 벡터·텍스트·톤·테두리·3D·효과 미리보기는 저장된 최종 합성본의
  원본 좌표를 비파괴로 잘라 표시하므로 CSP의 시각 결과를 유지합니다.
- CLIP/CSP의 저장된 래스터 mipmap, 벡터 스트로크, 용지, 기본 텍스트,
  단색·그레이디언트 채우기는 지원 데이터 범위에서 ON/OFF 재합성합니다.
- 독립 래스터가 저장되지 않은 CSP 전용 레이어는 CELSYS 렌더러 없이 다른 가시성
  조합으로 완전히 재합성할 수 없어 ON/OFF가 비활성화되며, 이미지 위치 기반
  레이어 선택은 지원하지 않습니다.
